...
// Component để tải template từ query parameters
const TemplateLoader = () => {
  ...
  useEffect(() => {
    ...
    if (template && !loaded && typeof template === "string") {
      try {
        const templateData = localStorage.getItem(
          `craftjs-template-${template}`
        );
        if (templateData) {
        ...
        console.error("Lỗi khi tải template từ query:", e);
      }
    }
  }, [query, loaded, actions]);
